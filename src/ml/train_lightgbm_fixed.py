"""
src/ml/train_lightgbm_fixed.py
Training script for the Baseline LightGBM model with CORRECTED vulnerability_age_days.

Design & Constraints:
  1. Uses redefined vulnerability_age_days (observation_date T - Published_Date).
  2. Preserves 180-day target definition and chronological splits.
  3. No EPSS, KEV, target-derived, or identifier features.
  4. scale_pos_weight calculated ONLY from training set.
  5. Early stopping based ONLY on validation set.
  6. Final evaluation ONCE on untouched test set.

Outputs:
  - Model: models/lightgbm_baseline_temporal_age_fixed.pkl
  - Predictions: outputs/baseline_predictions_temporal_age_fixed.csv
  - Report: reports/baseline_metrics_temporal_age_fixed.json
"""
...
def run_safety_checks(X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    print("=" * 65)
    print("  RUNNING PRE-TRAINING SAFETY CHECKS (TEMPORAL AGE FIXED)")
    print("=" * 65)
    ...
    print("=" * 65)
    print("  ALL SAFETY CHECKS PASSED -- PROCEEDING TO TRAIN FIXED MODEL")
    print("=" * 65 + "\n")
...
def train_baseline_fixed():
    ...
    # 8. Save Separate Outputs & Metrics
    os.makedirs("models", exist_ok=True)
    os.makedirs("outputs", exist_ok=True)
    os.makedirs("reports", exist_ok=True)

    model_path = "models/lightgbm_baseline_temporal_age_fixed.pkl"
    joblib.dump(model, model_path)
    print(f"  [Saved] Fixed model weights saved to: {model_path}")
    ...
    preds_path = "outputs/baseline_predictions_temporal_age_fixed.csv"
    preds_df.to_csv(preds_path, index=False)
    print(f"  [Saved] Fixed predictions saved to: {preds_path}")

    # Save metrics JSON report
    report_dict = {
        "model_name": "LightGBM_Baseline_Temporal_Age_Fixed",
    ...
    report_path = "reports/baseline_metrics_temporal_age_fixed.json"
    with open(report_path, "w") as f:
        json.dump(report_dict, f, indent=2)
    print(f"  [Saved] Fixed evaluation report saved to: {report_path}")
